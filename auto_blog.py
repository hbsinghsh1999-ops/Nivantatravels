...
def fetch_news_from_api(existing_titles, count=3):
    """Fetches articles dynamically from News API with strict travel search terms."""
    ...
    try:
        ...
        if not articles:
            print("No travel articles returned from News API.")
            return None, None
        ...
        if not selected_articles:
            print("All top travel stories have already been published. Skipping today.")
            return None, None
...
def generate_article_and_card(news_text, image_url):
    ...
    Review these raw news updates:
    {news_text}
    
    STRICT RELEVANCE CHECK:
    Are these news items strictly related to international travel, tourism, visas, flight guidelines, passport rules, or holiday taxes for Indian travelers?
    If the news is about generic corporate governance, stock market penalties, tea industries, mining, or non-travel politics, output strictly:
    {{"error": "NOT_TRAVEL_NEWS"}}

    Otherwise, output strictly valid JSON:
    {{
      "title": "Authoritative Travel Headline Here",
    ...
    try:
        ...
        if data.get("error") == "NOT_TRAVEL_NEWS":
            print("AI Filter Triggered: News is not related to travel. Skipping publication.")
            return None
